    ...
    <div className="flex h-[calc(100vh-8.5rem)] gap-4 animate-fade-in">
            ...
            {sessions.map((s) => (
              ...
              >
                ...
                <button
                  ...
                  className="opacity-0 group-hover:opacity-100 p-1 text-muted-custom hover:text-rose-500 transition-opacity"
      ...
      <div className="flex-1 flex flex-col rounded-xl border border-theme bg-card overflow-hidden">
        ...
        <div className="p-3.5 border-b border-theme bg-muted/50 flex items-center justify-between">
          ...
          {sessionId && (
            <span className="font-mono text-[10px] text-muted-custom bg-surface px-2.5 py-0.5 rounded border border-theme">
          ...
          {messages.length === 0 && !loading && (
            <div className="text-center space-y-2.5 py-16 text-muted-custom">
              ...
              <p className="font-semibold text-sm text-main">
                Ask any research question to start an evidence-grounded session.
              </p>
                  ...
                  <div>
                      ...
                      <div className="mt-3.5 pt-3 border-t border-theme space-y-2">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          {msg.citations.map((cite, cIdx) => (
                            <div
                              key={cIdx}
                              className="p-2 rounded-lg border border-theme bg-surface text-[11px] space-y-1"
                            >
                              <div className="font-semibold text-main truncate">
                                {cite.document_name} <span className="text-muted-custom font-normal font-mono">(p. {cite.page_number})</span>
                              </div>
                    ...
                    {msg.timing_breakdown && (
                      <div className="mt-3 pt-2">
                        ...
                        {showDevTiming[idx] && (
                          <div className="mt-2 p-2.5 rounded-lg bg-surface font-mono text-[10px] text-sub border border-theme space-y-1">
                            <div>Total Latency: {msg.latency}s</div>
                            <div>LLM Generation: {msg.llm_latency}s</div>
                            {msg.timing_breakdown.ms && (
                              <pre className="text-[9px] text-emerald-600 dark:text-emerald-400 pt-1">
