...
import { Search, Hash, Clock, FileText, Layers, Loader2, AlertCircle, Sliders } from 'lucide-react';
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
          ...
          BM25 Retrieval Tester
        </h1>
        <p className="text-slate-500 dark:text-slate-400 text-xs">
          Developer testing view to evaluate sparse search recall and Okapi BM25 scoring. Query matches directly against tokenized document chunks.
        </p>
      </div>
        ...
        <form onSubmit={handleSearch} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          <div className="md:col-span-2 space-y-1.5">
            ...
            <input
              ...
              placeholder="Type keyword terms here..."
      ...
      {results && (
        <div className="flex flex-wrap items-center gap-4 text-xs text-slate-500 dark:text-slate-400 py-1 px-2 bg-slate-50 dark:bg-slate-800/20 border border-slate-200 dark:border-slate-800 rounded-lg">
          <span className="flex items-center gap-1 font-medium">
            ...
            Retrieval latency: <strong className="text-slate-700 dark:text-slate-200 font-bold">{results.latency.toFixed(4)}s</strong>
          </span>
      ...
      {results && results.chunks.length === 0 && (
        <div className="text-center py-16 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl space-y-2">
          ...
          <p className="text-xs text-slate-400">The query returned no BM25 matches. Ensure you have ingested documents in your library and search for exact keywords.</p>
        </div>
      )}
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 dark:border-slate-800/60 pb-2.5">
                <div className="flex flex-wrap items-center gap-3 text-xs">
                  ...
                  {chunk.retrieval_type && (
                    <span className="bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300 px-2 py-0.5 rounded font-semibold text-[10px] uppercase tracking-wider">
                      {chunk.retrieval_type}
                    </span>
                  )}
                ...
                <div className="flex items-center gap-1.5 text-xs font-semibold">
                  <span className="text-slate-400 dark:text-slate-500">BM25 Score:</span>
