...
import { Search, Hash, Clock, FileText, Layers, Loader2, AlertCircle, ChevronsUp, Cpu, Shuffle } from 'lucide-react';
    ...
    try {
      ...
      if (response.success) {
        setResults({
          ...
          hybridLatency: response.hybrid_retrieval_time_seconds,
          rerankLatency: response.rerank_time_seconds,
          chunks: response.results || []
        });
      } else {
        setError(response.message || "Rerank search failed.");
      }
    } catch (err) {
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
          <ChevronsUp className="w-6 h-6 text-brand-500" />
          Cross-Encoder Reranking
        </h1>
        <p className="text-slate-500 dark:text-slate-400 text-xs">
          Developer testing view to evaluate Cross-Encoder Reranking layer. Reranks top candidates from Hybrid Retrieval using `cross-encoder/ms-marco-MiniLM-L-6-v2` for optimal context relevance.
        </p>
      </div>
        ...
        <form onSubmit={handleSearch} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          <div className="md:col-span-2 space-y-1.5">
            ...
            <input
              ...
              placeholder="Type your query to test reranking..."
      ...
      {results && (
        <div className="flex flex-wrap items-center gap-4 text-xs text-slate-500 dark:text-slate-400 py-1.5 px-3 bg-slate-50 dark:bg-slate-800/20 border border-slate-200 dark:border-slate-800 rounded-lg">
          <span className="flex items-center gap-1 font-medium">
            <Clock className="w-3.5 h-3.5 text-indigo-500" />
            Total: <strong className="text-slate-700 dark:text-slate-200 font-bold">{results.latency.toFixed(4)}s</strong>
          </span>
          <span className="text-slate-300 dark:text-slate-700">|</span>
          <span className="flex items-center gap-1 font-medium">
            <Shuffle className="w-3.5 h-3.5 text-purple-500" />
            Hybrid retrieve: <strong className="text-slate-700 dark:text-slate-200 font-bold">{results.hybridLatency.toFixed(4)}s</strong>
          </span>
          <span className="text-slate-300 dark:text-slate-700">|</span>
          <span className="flex items-center gap-1 font-medium">
            <Cpu className="w-3.5 h-3.5 text-amber-500" />
            Cross Encoder: <strong className="text-slate-700 dark:text-slate-200 font-bold">{results.rerankLatency.toFixed(4)}s</strong>
          </span>
          <span className="text-slate-300 dark:text-slate-700">|</span>
          <span className="flex items-center gap-1 font-medium">
            <Hash className="w-3.5 h-3.5 text-indigo-500" />
            Returned: <strong className="text-slate-700 dark:text-slate-200 font-bold">{results.chunks.length} chunks</strong>
          </span>
        </div>
      )}
      ...
      {results && results.chunks.length === 0 && (
        <div className="text-center py-16 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl space-y-2">
          ...
          <p className="text-xs text-slate-400">The query returned no results. Ensure you have ingested documents in your library.</p>
        </div>
      )}
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 dark:border-slate-800/60 pb-2.5">
                ...
                <div className="flex items-center gap-3 text-xs font-semibold">
                  <div className="flex items-center gap-1">
                    <span className="text-slate-400 dark:text-slate-500">Hybrid:</span>
                    <span className="text-slate-700 dark:text-slate-300 font-mono">
                      {chunk.score.toFixed(6)}
                    </span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <span className="text-slate-400 dark:text-slate-500">Rerank Score:</span>
                    <span className="text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 px-2.5 py-0.5 rounded-full font-bold">
                      {chunk.rerank_score.toFixed(6)}
                    </span>
                  </div>
                </div>
              </div>
